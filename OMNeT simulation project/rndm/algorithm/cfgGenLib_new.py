...
def generate_all_config_files(cfgName, baseCfgName, hostNumbers, hostPrios, hostIPs, serverIPs, bwSplits, ceilSplits,leafBwSplit,leafCeilSplit, rate,routingNumbers, routingIPs,hostNumbersRouting,useTwoLevelHTB):
    if not useTwoLevelHTB:
        ...
        print("In generate_all_config_files")
        # ini - for regular and anarcho version.
        generate_ini(cfgName, baseCfgName, hostNumbers, hostNumbersRouting,hostIPs, bwSplits,rate,twoLevelHTB = True)
        ...
        for bwSplit,ceilSplit in zip(bwSplits,ceilSplits):
            longName = "%s%s" % (cfgName, "".join(["%s%02d" % (k, round(v / 1e5)) for k, v in bwSplit.items()]))
            print(leafBwSplit[0])
            generate_htb_xml_two_level(cfgName, hostNumbers, hostPrios, bwSplit, ceilSplit,leafBwSplit[0],leafCeilSplit[0])
