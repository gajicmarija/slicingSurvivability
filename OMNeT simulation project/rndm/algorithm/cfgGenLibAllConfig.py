...
def generate_ini(cfgName, baseCfgName, hostNumbers, clientsNumbers, hostIPs, bwSplits, rate,withoutHTB = False, twoLevelHTB = False):
    # General config.
...
def generate_all_config_files1(cfgName, baseCfgName, hostNumbers, hostPrios, hostIPs, serverIPs, bwSplits, ceilSplits, rate):

    # ini - for regular and anarcho version.
    print("In generate_all_config_files")
    print(rate)
    generate_ini1(cfgName, baseCfgName, hostNumbers,hostIPs, bwSplits,rate)
    ...
    for bwSplit,ceilSplit in zip(bwSplits,ceilSplits):
        longName = "%s%s" % (cfgName, "".join(["%s%02d" % (k, round(v / 1e5)) for k, v in bwSplit.items()]))
        generate_htb_xml1(cfgName, hostNumbers, hostPrios, bwSplit, ceilSplit,rate)
        generate_routing_xml1(cfgName, hostNumbers, hostIPs, serverIPs)

def generate_ini1(cfgName, baseCfgName, hostNumbers, hostIPs, bwSplits,rate,withoutHTB = False, twoLevelHTB = False):
    # General config.
    print(rate)
    print("rate above is from ini")
    for bwSplit in bwSplits:
        longName = "%s%s" % (cfgName, "".join(["%s%02d" % (k, round(v / 1e5)) for k, v in bwSplit.items()]))
        if not rate:
            totalRate = sum(bwSplit.values())
    ...
    # Routing and hosts.
            ...
            # Rate and delay.
            cfgString = cfgString + '**.connFIX0.datarate = %d bps\n' % totalRate
            cfgString = cfgString + '**.connFIX0.delay = 10ms\n\n'
...
def generate_htb_xml1(cfgName, hostNumbers, hostPrios, bwSplit,ceilSplit,rate):
    if not rate:
            totalRate = sum(bwSplit.values())
    else:
            totalRate = rate
...
def generate_routing_xml1(cfgName, hostNumbers, hostIPs, serverIPs, twoLevelHTB = False):
    configElem = ET.Element('config')
    activeHosts = [k for k in hostNumbers.keys() if hostNumbers[k] > 0]
    # Host IPs.
    for h in activeHosts:
...
def generate_all_config_files2(cfgName, baseCfgName, hostNumbers, hostPrios, hostIPs, serverIPs, bwSplits, ceilSplits,flowNumbers,rate=0):

    # ini - for regular and anarcho version.
    print("In generate_all_config_files")
    generate_ini2(cfgName, baseCfgName, hostNumbers,hostIPs, bwSplits,flowNumbers,rate)
    #generate_ini('%s' % cfgName, baseCfgName, hostNumbers, hostIPs, bwSplits,rate, withoutHTB = True)
    # routing.xml - skip types with number == 0?
    # htb.xml (1 per bwSplit) - skip with number == 0
    for bwSplit,ceilSplit in zip(bwSplits,ceilSplits):
        longName = "%s%s" % (cfgName, "".join(["%s%02d" % (k, round(v / 1e5)) for k, v in bwSplit.items()]))
        generate_htb_xml2(cfgName, hostNumbers, hostPrios, bwSplit, ceilSplit,rate)
        generate_routing_xml2(cfgName, flowNumbers, hostIPs, serverIPs)

def generate_ini2(cfgName, baseCfgName, hostNumbers, hostIPs, bwSplits, flowNumbers, rate=0, withoutHTB = False, twoLevelHTB = False):
    # General config.
    ...
    # Routing and hosts.
            ...
            if not withoutHTB:
                ...
                r0pdf = []
                r1pdf = []
                #***this is for critical and non critical two slices***
                r0pdf.append('[expr(ipv4.srcAddress.str() =~ "10.5.*.*" ||  ipv4.srcAddress.str() =~ "10.6.*.*" ||  ipv4.srcAddress.str() =~ "10.7.*.*") ,expr(ipv4.srcAddress.str() =~ "10.0.*.*" || ipv4.srcAddress.str() =~ "10.1.*.*" || ipv4.srcAddress.str() =~ "10.2.*.*" || ipv4.srcAddress.str() =~ "10.3.*.*" || ipv4.srcAddress.str() =~ "10.4.*.*")]')
                r1pdf.append('[expr(ipv4.destAddress.str() =~ "10.5.*.*" ||  ipv4.destAddress.str() =~ "10.6.*.*" ||  ipv4.destAddress.str() =~ "10.7.*.*"),expr(ipv4.destAddress.str() =~ "10.0.*.*" || ipv4.destAddress.str() =~ "10.1.*.*" || ipv4.destAddress.str() =~ "10.2.*.*" || ipv4.destAddress.str() =~ "10.3.*.*" || ipv4.destAddress.str() =~ "10.4.*.*")]')

                #this is for delay and bandwidth
                #r0pdf.append('[expr(ipv4.srcAddress.str() =~ "10.0.*.*" ||  ipv4.srcAddress.str() =~ "10.4.*.*" ||  ipv4.srcAddress.str() =~ "10.5.*.*") ,expr(ipv4.srcAddress.str() =~ "10.1.*.*" || ipv4.srcAddress.str() =~ "10.2.*.*" || ipv4.srcAddress.str() =~ "10.3.*.*" || ipv4.srcAddress.str() =~ "10.6.*.*" || ipv4.srcAddress.str() =~ "10.7.*.*")]')
                #r1pdf.append('[expr(ipv4.destAddress.str() =~ "10.0.*.*" ||  ipv4.destAddress.str() =~ "10.4.*.*" ||  ipv4.destAddress.str() =~ "10.5.*.*"),expr(ipv4.destAddress.str() =~ "10.1.*.*" || ipv4.destAddress.str() =~ "10.2.*.*" || ipv4.destAddress.str() =~ "10.3.*.*" || ipv4.destAddress.str() =~ "10.6.*.*" || ipv4.destAddress.str() =~ "10.7.*.*")]')
            ...
            #runfile.close()
                ...
                #r0pdf.append("[expr(ipv4.srcAddress.str() =~ "10.0.*.*" || ipv4.srcAddress.str() =~ "10.1.*.*" || ipv4.srcAddress.str() =~ "10.2.*.*" || ipv4.srcAddress.str() =~ "10.3.*.*" || ipv4.srcAddress.str() =~ "10.4.*.*"), expr(ipv4.srcAddress.str() =~ "10.5.*.*" ||  ipv4.srcAddress.str() =~ "10.6.*.*" ||  ipv4.srcAddress.str() =~ "10.7.*.*")]")
                #r1pdf.append("[expr(ipv4.destAddress.str() =~ "10.0.*.*" || ipv4.destAddress.str() =~ "10.1.*.*" || ipv4.destAddress.str() =~ "10.2.*.*" || ipv4.destAddress.str() =~ "10.3.*.*" || ipv4.destAddress.str() =~ "10.4.*.*"), expr(ipv4.destAddress.str() =~ "10.5.*.*" ||  ipv4.destAddress.str() =~ "10.6.*.*" ||  ipv4.destAddress.str() =~ "10.7.*.*")]")

def generate_htb_xml2(cfgName, hostNumbers, hostPrios, bwSplit,ceilSplit,rate=0):
    if not rate: 
        totalRate = sum(bwSplit.values())
    else:
        totalRate=rate    
...
def generate_routing_xml2(cfgName, hostNumbers, hostIPs, serverIPs, twoLevelHTB = False):
    configElem = ET.Element('config')
    activeHosts = [k for k in hostNumbers.keys() if hostNumbers[k] > 0]
    # Host IPs.
    for h in activeHosts:
...
def generate_all_config_files3(cfgName, baseCfgName, hostNumbers, hostPrios, hostIPs, serverIPs, bwSplits, ceilSplits,flowNumbers,rate=0):

    # ini - for regular and anarcho version.
    print("In generate_all_config_files")
    generate_ini3(cfgName, baseCfgName, hostNumbers,hostIPs, bwSplits,flowNumbers,rate)
    #generate_ini('%s' % cfgName, baseCfgName, hostNumbers, hostIPs, bwSplits,rate, withoutHTB = True)
    # routing.xml - skip types with number == 0?
    # htb.xml (1 per bwSplit) - skip with number == 0
    for bwSplit,ceilSplit in zip(bwSplits,ceilSplits):
        longName = "%s%s" % (cfgName, "".join(["%s%02d" % (k, round(v / 1e5)) for k, v in bwSplit.items()]))
        generate_htb_xml3(cfgName, hostNumbers, hostPrios, bwSplit, ceilSplit,rate)
        generate_routing_xml3(cfgName, flowNumbers, hostIPs, serverIPs)

def generate_ini3(cfgName, baseCfgName, hostNumbers, hostIPs, bwSplits, flowNumbers, rate=0, withoutHTB = False, twoLevelHTB = False):
    # General config.
    ...
    # Routing and hosts.
            ...
            if not withoutHTB:
                ...
                #***this is for 4 slices, where each critical application has its own slice and all non critical apps are in one slice ***
                #r0pdf.append('[expr(ipv4.srcAddress.str() =~ "10.5.*.*"), expr(ipv4.srcAddress.str() =~ "10.6.*.*"), expr(ipv4.srcAddress.str() =~ "10.7.*.*") ,expr(ipv4.srcAddress.str() =~ "10.0.*.*" || ipv4.srcAddress.str() =~ "10.1.*.*" || ipv4.srcAddress.str() =~ "10.2.*.*" || ipv4.srcAddress.str() =~ "10.3.*.*" || ipv4.srcAddress.str() =~ "10.4.*.*")]')
                #r1pdf.append('[expr(ipv4.destAddress.str() =~ "10.5.*.*"), expr(ipv4.destAddress.str() =~ "10.6.*.*"), expr(ipv4.destAddress.str() =~ "10.7.*.*") ,expr(ipv4.destAddress.str() =~ "10.0.*.*" || ipv4.destAddress.str() =~ "10.1.*.*" || ipv4.destAddress.str() =~ "10.2.*.*" || ipv4.destAddress.str() =~ "10.3.*.*" || ipv4.destAddress.str() =~ "10.4.*.*")]')




                #this is for 4 slices, DEL CRIT AND NON CRIT AND BW CRIT AND NON CRIT
                r0pdf.append('[expr(ipv4.srcAddress.str() =~ "10.5.*.*"), expr(ipv4.srcAddress.str() =~ "10.0.*.*" || ipv4.srcAddress.str() =~ "10.4.*.*") ,expr(ipv4.srcAddress.str() =~ "10.6.*.*" || ipv4.srcAddress.str() =~ "10.7.*.*" ),expr(ipv4.srcAddress.str() =~ "10.1.*.*" || ipv4.srcAddress.str() =~ "10.2.*.*" || ipv4.srcAddress.str() =~ "10.3.*.*" )]')
                r1pdf.append('[expr(ipv4.destAddress.str() =~ "10.5.*.*"), expr(ipv4.destAddress.str() =~ "10.0.*.*" || ipv4.destAddress.str() =~ "10.4.*.*"),expr(ipv4.destAddress.str() =~ "10.6.*.*" || ipv4.destAddress.str() =~ "10.7.*.*"), expr(ipv4.destAddress.str() =~ "10.1.*.*" || ipv4.destAddress.str() =~ "10.2.*.*" || ipv4.destAddress.str() =~ "10.3.*.*")]') 
...
def generate_htb_xml3(cfgName, hostNumbers, hostPrios, bwSplit,ceilSplit,rate=0):
    if not rate: 
        totalRate = sum(bwSplit.values())
    else:
        totalRate=rate    
...
def generate_routing_xml3(cfgName, hostNumbers, hostIPs, serverIPs, twoLevelHTB = False):
    configElem = ET.Element('config')
    activeHosts = [k for k in hostNumbers.keys() if hostNumbers[k] > 0]
    # Host IPs.
    for h in activeHosts:
