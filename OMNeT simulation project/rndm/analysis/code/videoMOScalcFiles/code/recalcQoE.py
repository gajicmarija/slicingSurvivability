...
def createTempCSV(testName, numCLI, nodeTypes, nodeSplit, nodeType, nodeNum):
    ...
    for col in colList[::2]:
        ...
        print(df.columns.get_loc(df.filter(like=valName).columns[0]))
...
if __name__ == "__main__":
    ...
    numCLI = numVID + numLVD + numFDO + numSSH + numVIP + numcVP + numcF + numcLV
    #recalculateQoEsimulationRun(sys.argv[1], numCLI, ['hostVID', 'hostLVD', 'hostFDO', 'hostSSH', 'hostVIP','hostcVP','hostcF','hostcLV'],  [numVID, numLVD, numFDO, numSSH, numVIP,numcVP, numcF, numcLV], 'hostLVD', 1)
    #recalculateQoEsimulationRun(sys.argv[1], numCLI, ['hostVID', 'hostLVD', 'hostFDO', 'hostSSH', 'hostVIP','hostcVP','hostcF','hostcLV'],  [numVID, numLVD, numFDO, numSSH, numVIP,numcVP, numcF, numcLV], 'hostVID', 5)
    recalculateQoEsimulationRun(sys.argv[1], numCLI, ['hostVID', 'hostLVD', 'hostFDO', 'hostSSH', 'hostVIP','hostcVP','hostcF','hostcLV'],  [numVID, numLVD, numFDO, numSSH, numVIP,numcVP, numcF, numcLV], 'hostcLV', 1)
