...
def partialCDFPlotData(fig, ax, data, label, lineType, lineColor):
    #OrderedDict([('hostVID', 3.217232), ('hostLVD', 3.34328), ('hostFDO', 2.6982), ('hostSSH', 0.000408), ('hostVIP', 0.0108), ('hostcVP', 0.0108), ('hostcF', 2.6982), ('hostcLV', 3.443664)])
    sorted_data = np.sort(data)
    y = 1. * np.arange(len(sorted_data)) / (len(sorted_data) - 1)
    z=np.full(y.shape, 0.2)
    idx = np.argwhere(np.diff(np.sign(y - z))).flatten()
    ax.plot(sorted_data, y, lineType, label=label, color=lineColor)
    #ax.plot(sorted_data, z, lineType, label=label, color=lineColor)
    ax.plot(sorted_data[idx], y[idx], 'bx')
    counterUnderThreshold = 0
    for i in range(0, len(sorted_data)):
        if float(sorted_data[i]) < float(sorted_data[idx]):
            counterUnderThreshold+=1
            #print(counterUnderThreshold)
    print(float(counterUnderThreshold/len(sorted_data))*100)
    #print(counterUnderThreshold)
    #print(len(sorted_data))
    return float(sorted_data[idx])

def getProbabilities(fig, ax, data, label, lineType, lineColor, hostType, missingDataVIP, missingDatacVIP):
    threshold30=OrderedDict([('hostVID', 3.217232), ('hostLVD', 3.34328), ('hostFDO', 2.6982), ('hostSSH', 0.000408), ('hostVIP', 0.0108), ('hostcVP', 0.0108), ('hostcF', 2.6982), ('hostcLV', 3.443664)])
    threshold20=OrderedDict([('hostVID', 0.952), ('hostLVD', 1.092), ('hostFDO', 2.224), ('hostSSH', 0.005), ('hostVIP', 0.018), ('hostcVP', 0.018), ('hostcF', 2.224), ('hostcLV', 1.092)]) #those are thresholds from CNSM  apart from VoIP and cVoIP

    sorted_data = np.sort(data)
    y = 1. * np.arange(len(sorted_data)) / (len(sorted_data) - 1)
    if hostType== "hostSSH":
        print(sorted_data)
    print(statistics.mean(sorted_data))
    #z=np.full(y.shape, 0.3)
    #idx = np.argwhere(np.diff(np.sign(y - z))).flatten()
    #ax.plot(sorted_data, y, lineType, label=label, color=lineColor)
    #ax.plot(sorted_data, z, lineType, label=label, color=lineColor)
    #ax.plot(sorted_data[idx], y[idx], 'bx')
    counterUnderThreshold = 0
    for i in range(0, len(sorted_data)):
        if float(sorted_data[i]) < threshold20[hostType]:
            counterUnderThreshold+=1
            #print(counterUnderThreshold)
    if(hostType=="hostVIP"):
        print("getProbabilities hostVIP missisng data")
        print(float((counterUnderThreshold+missingDataVIP)/len(sorted_data))*100)
        return float((counterUnderThreshold+missingDataVIP)/len(sorted_data))*100
    if(hostType=="hostcVP"):
        print("getProbabilities hostcVIP")
        print(float((counterUnderThreshold+missingDatacVIP)/len(sorted_data))*100)
        return float((counterUnderThreshold+missingDatacVIP)/len(sorted_data))*100
    else:
        print(float(counterUnderThreshold/len(sorted_data))*100)
        return float(counterUnderThreshold/len(sorted_data))*100
    #print(counterUnderThreshold)
    #print(len(sorted_data))

def getProbabilitiesMean(fig, ax, data, label, lineType, lineColor, hostType):
    threshold30=OrderedDict([('hostVID', 3.217232), ('hostLVD', 3.34328), ('hostFDO', 2.6982), ('hostSSH', 0.000408), ('hostVIP', 0.0108), ('hostcVP', 0.0108), ('hostcF', 2.6982), ('hostcLV', 3.443664)])
    sorted_data = np.sort(data)
    y = 1. * np.arange(len(sorted_data)) / (len(sorted_data) - 1)
    #z=np.full(y.shape, 0.3)
    ...
    counterUnderThreshold = 0
    for i in range(0, len(sorted_data)):
        if float(sorted_data[i]) < threshold30[hostType]:
            counterUnderThreshold+=1
            #print(counterUnderThreshold)
    print(float(counterUnderThreshold/len(sorted_data))*100)
    #print(counterUnderThreshold)
...
def chooseName(dataName):
    if dataName == 'hostVID':
        ...
        return 'Live Video'
    elif dataName == 'hostcLV':
        return 'crit Live Video'
    elif dataName == 'hostcVP':
        return 'crit VoIP'
    elif dataName == 'hostcF':
        return 'crit FD'
...
def chooseColor(dataName):
    ...
    if dataName == 'hostcVP':
        return colorMapping['cVP']
    if dataName == 'hostcF':
        return colorMapping['cF']
    if dataName == 'hostcLV':
        return colorMapping['cLV']    
...
def plotDataTypeCdfAllApps(testName, numCLI, nodeTypes, nodeSplit, dataIdent, folderName, nodeTypesToPlot):
    ...
    for nodeType,numNodes in zip(nodeTypes,nodeSplit):
        if nodeType in nodeTypesToPlot:
            tempValue = []
             #['hostVID', 'hostLVD', 'hostFDO', 'hostSSH', 'hostVIP']
            for nodeNum in range(numNodes):
                colName = makeNodeIdentifier(nodeType, nodeNum) + " " + dataIdent + " Val"
                tempValue.extend(df[colName].dropna().tolist())
                allClients.extend(df[colName].dropna().tolist())
            if dataIdent == 'Mos':
                print(sum(tempValue) / len(tempValue))
                print(len(tempValue))
            ...
            if len(tempValue) > 0:
                ...
                partialCDFPlotData(fig, ax1, tempValue, chooseName(nodeType), '-o', chooseColor(nodeType))
    if dataIdent == 'Mos':
        print("Average MOS: %s" % (sum(allClients) / len(allClients)))   
...
# def exportMeanDataPerApp(testNameMain, testNameSecondary, numCLI, nodeTypes, nodeSplit, dataIdent, folderName, nodeTypesToPlot):
...
#             tempValue2 = []
#             meanPerApplication = []
#             meanPerApp = testNameMain + "," + nodeType + "," 
#             for nodeNum in range(numNodes):
#                 colName = makeNodeIdentifier(nodeType, nodeNum) + " " + dataIdent + " Val"
#                 data = df[colName].dropna().tolist()
#                 data2 = df2[colName].dropna().tolist()
#                 if len(data) > 0:
#                     tempValue.append(statistics.mean(data)) #tempvalue is holding the array of all the per client mean data!
#                     meanPerApplication.append(statistics.mean(data))
#                 if len(data2) > 0:
#                     print("------IN WEIRD IF, LINE 471.-----")
#                     tempValue2.append(statistics.mean(data2))
#                     meanPerApplication.append(statistics.mean(data))
#                 if dataIdent=nodeNum== (numNodes-1)  and len(meanPerApplication) > 0:
#                     meanPerApp+=str(statistics.mean(meanPerApplication))
#                     f = open('../extracted/meanTPperApp/meanTPperApp.csv', 'a')
#                     f.write(meanForApplication)
...
#this hear plots timeseries of the TPs, named number 5 in the plots
def plotTPdirection(testName, numCLI, nodeTypes, nodeSplit, numSlices, direction):
    ...
    print(nodeTypes)
    print(nodeSplit)
    #nodeSplit1 =   [135, 37, 60, 100, 350, 100, 20, 18]
    #nodeTypes1 = ['hostVID', 'hostLVD', 'hostFDO', 'hostSSH', 'hostVIP', 'hostcVP', 'hostcF', 'hostcLV']
    #nodeSplit1 =   [ 100, 20, 18]
    #nodeTypes1 = ['hostcVP', 'hostcF', 'hostcLV']
    nodeSplit1 =   [135, 37, 60, 100, 350]
    nodeTypes1 = ['hostVID', 'hostLVD', 'hostFDO', 'hostSSH', 'hostVIP']

    print(nodeSplit)
    totalDF = pd.DataFrame()
    criticalDF = pd.DataFrame()
    nonCriticalDF = pd.DataFrame()
    for nodeType,numNodes in zip(nodeTypes,nodeSplit):
        tempDF = pd.DataFrame()
        ...
        totalDF = pd.concat([totalDF,tempDF],axis=1,ignore_index=False)
        print(nodeType)
        if nodeType == "hostcVP" or nodeType == "hostcLV" or nodeType == "hostcF":
            criticalDF = pd.concat([criticalDF,tempDF],axis=1,ignore_index=False)
            print(criticalDF)
        else:  
            nonCriticalDF = pd.concat([nonCriticalDF,tempDF],axis=1,ignore_index=False)
            #print(tempDF) 
        ax1.plot(times, [x/1000 for x in tempDF.sum(axis=1).tolist()], label=chooseName(nodeType), marker='o', ls='-', color=chooseColor(nodeType))
    ax1.plot(times, [x/1000 for x in totalDF.sum(axis=1).tolist()], label="All", marker='o', ls='-', color=colormap[8])
    #ax1.plot(times, [x/1000 for x in criticalDF.sum(axis=1).tolist()], label="Critical", marker='.', ls='-.', color=colormap[8])   
    ax1.plot(times, [x/1000 for x in nonCriticalDF.sum(axis=1).tolist()], label="nonCritical", marker='.', ls='-.', color=colormap[8])
...
def plotTPScdfDirection(testName, numCLI, nodeTypes, nodeSplit, numSlices, direction, cutoff):
    ...
    fig, ax1 = partialCDFBegin(1)
    maxTPS = 12
    nodeSplit1 = [1,1,1,1,1,1,1,1]
    nodeTypes1 = ['hostVID','hostcF','hostFDO']
    print("plotTPScdfDirection")
    for nodeType,numNodes in zip(nodeTypes,nodeSplit):
        tempTPSall = []
        for nodeNum in range(numNodes):
            colName = direction[0] + " Throughput " + makeNodeIdentifier(nodeType, nodeNum)
...
def meanAppTP(testName, numCLI, nodeTypes, nodeSplit, direction, cutoff):
    df = importDFextended(testName, numCLI, nodeTypes, nodeSplit, 'throughputs', '_' + direction[0])
    fig, ax1 = partialCDFBegin(1)
    maxTPS = 12
    for nodeType,numNodes in zip(nodeTypes,nodeSplit):
        tempTPSall = []
        meanPerApp = testName + "," + nodeType + "," 
        print(numNodes)
        for nodeNum in range(numNodes):
            colName = direction[0] + " Throughput " + makeNodeIdentifier(nodeType, nodeNum)
            tempTPSall.append(statistics.mean([x/1000 for x in df[colName].tolist()[:int(cutoff)+1]]))
            ...
            if nodeNum==(numNodes-1):
               #[135, 37, 60, 100, 350, 100, 20, 18]
                meanPerApp+=str(statistics.mean(tempTPSall))
                meanPerApp+=","
                meanPerApp+=str(statistics.stdev(tempTPSall))
                meanPerApp+="\n"
                f = open('/Users/marijagajic/omnetpp-6.0pre15/samples/rndm/analysis/exports/extracted/meanTPperApp/meanTPperApp.csv', 'a')
                f.write(meanPerApp)
            #runfile.write("./runAndExportSimConfigWithCleanupSH.sh -i omnetpp.ini -c %s -s 5\n" % cfgName )
                f.close()
        #if maxTPS < max(tempTPSall):
...
#[135, 37, 60, 100, 350, 100, 20, 18]
def meanAppDelay(testName, numCLI, nodeTypes, nodeSplit, cutoff):
    #df = importDF(testNameMain, numCLI, nodeTypes, nodeSplit, folderName)
    df = importDF(testName, numCLI, nodeTypes, nodeSplit, 'endToEndDelay' )
    fig, ax1 = partialCDFBegin(1)
    maxTPS = 12
    for nodeType,numNodes in zip(nodeTypes,nodeSplit):
        tempTPSall = []
        ...
        for nodeNum in range(numNodes):
            ...
            if nodeNum==(numNodes-1):
               #[135, 37, 60, 100, 350, 100, 20, 18]
                ...
                meanPerApp+=str(statistics.stdev(tempTPSall))
                meanPerApp+="\n"
                f = open('/Users/marijagajic/omnetpp-6.0pre15/samples/rndm/analysis/exports/extracted/meanDelayPerApp/meanDelayPerApp.csv', 'a')
                f.write(meanPerApp)
            #runfile.write("./runAndExportSimConfigWithCleanupSH.sh -i omnetpp.ini -c %s -s 5\n" % cfgName )
                f.close()
        #if maxTPS < max(tempTPSall):
...
def plotTPS(testName, numCLI, nodeTypes, nodeSplit, numSlices, direction, cutoff):
    ...
    meanAppTP(testName, numCLI, nodeTypes, nodeSplit, direction, cutoff)
    globalCounter += 1
...
def calculateMOS(testName, numCLI, nodeTypes, nodeSplit, dataIdent, folderName, nodeTypesToPlot):
    df = importDF(testName, numCLI, nodeTypes, nodeSplit, folderName)
    fig, ax1 = partialCDFBegin(1)
    maxValue = 0
    averages =[]
    ...
    for nodeType,numNodes in zip(nodeTypes,nodeSplit):
        if nodeType in nodeTypesToPlot:
            ...
            for nodeNum in range(numNodes):
                colName = makeNodeIdentifier(nodeType, nodeNum) + " " + dataIdent + " Val"
                tempValue.extend(df[colName].dropna().tolist()) #column for each client
                allClients.append(float(sum(tempValue) / len(tempValue))) #add mean for each client

            if dataIdent == 'Mos':
                print(nodeType)
                print(sum(tempValue) / len(tempValue)) #mean of all clients
                row.append(sum(tempValue) / len(tempValue))
                print(len(tempValue)) #should be equal to total number of clients
    ...
    if dataIdent == 'Mos':
        ax1.set_xlim(0.95,5.05)
        print(sum(allClients) / len(allClients)) #mean of all clients
        row.append(sum(allClients) / len(allClients))
        
        with open('../exports/ml_data/output.csv','a') as fd:
            write = csv.writer(fd) 
            write.writerow(row) 
            #fd.writerow(", ".join( repr(e) for e in row) )
        print(len(allClients)) #should be equal to total number of clients
    else:
        ax1.set_xlim(0,1.01*maxValue)
    partialCDFEnd(fig,ax1,'', 'Client ' + niceDataTypeName[dataIdent], '../exports/plots/'+makeFullScenarioName(testName, numCLI, nodeTypes, nodeSplit)+'/'+str(globalCounter)+'_cdf' + dataIdent + str(nodeTypesToPlot) + '.pdf')
    partialCDFEndPNG(fig,ax1,'', 'Client ' + niceDataTypeName[dataIdent], '../exports/plots/'+makeFullScenarioName(testName, numCLI, nodeTypes, nodeSplit)+'/'+str(globalCounter)+'_cdf' + dataIdent + str(nodeTypesToPlot) + '.png')
...
def plotAll(testName, compTestName, nodeTypes, nodeSplit, numSlices, cutoff):
    ...
    globalCounter = 0
    numCLI = sum(nodeSplit)
    #meanAppTP(testName, numCLI, nodeTypes, nodeSplit, downlink, cutoff)
    #meanAppDelay(testName, numCLI, nodeTypes, nodeSplit, cutoff)
    #sfindingTPmeanAppTP(testName, numCLI, nodeTypes, nodeSplit, downlink,cutoff)
    #meanProb(testName, numCLI, nodeTypes, nodeSplit, downlink,cutoff)
    #countMissingTP(testName,numCLI,nodeTypes,nodeSplit,downlink)
    #print(testName + ': Plotting VoIP Mean End-To-End Delay CDF...')
    meanDelayProbabilityV2(testName, numCLI, nodeTypes, nodeSplit, 'E2ED', 'endToEndDelay', ['hostSSH', 'hostVIP','hostcVP'])
    #mosProbability(testName, numCLI, ["hostVID",'hostLVD', 'hostFDO', 'hostSSH', 'hostVIP','hostcVP','hostcF','hostcLV'], [135,37,60,100,350,100,20,18], 'Mos', 'mos2', ['hostLVD', 'hostFDO', 'hostSSH', 'hostVIP','hostcVP','hostcF','hostcLV'])
    #new from here 
 



def findingTPmeanAppTP(testName, numCLI, nodeTypes, nodeSplit, direction, cutoff):
    df = importDFextended(testName, numCLI, nodeTypes, nodeSplit, 'throughputs', '_' + direction[0])
    fig, ax1 = partialCDFBegin(1)
    maxTPS = 12
    threshold=[]
    appThreshold = OrderedDict()
    missingDataVIP = 0 # this is VIP and cVIP clients that never get started
    missingDatacVIP = 0
    probabilityPerApp = testName + "," 
    for nodeType,numNodes in zip(nodeTypes,nodeSplit):
        tempTPSall = []
        print("-----------------------" + nodeType)
        ...
        for nodeNum in range(numNodes):
            ...
            colName = direction[0] + " Throughput " + makeNodeIdentifier(nodeType, nodeNum)
            #print(df[colName])
            if not countMissingTP(testName,numCLI,nodeTypes,nodeSplit,nodeType,nodeNum,direction):
                tempTPSall.extend([x/1000 for x in df[colName].tolist()[:int(cutoff)+1]])

        #print(tempTPSall)
            if nodeType == "hostVIP":
                missingDataVIP+=countMissingTP(testName,numCLI,nodeTypes,nodeSplit,nodeType,nodeNum,direction)
                #print(missingDataVIP)
            elif nodeType == "hostcVP":
                missingDatacVIP+=countMissingTP(testName,numCLI,nodeTypes,nodeSplit,nodeType,nodeNum,direction)
        #appThreshold[nodeType] = partialCDFPlotData(fig, ax1, tempTPSall, chooseName(nodeType), '-o', chooseColor(nodeType))
        print("Missing data VIP" + str(missingDataVIP))
        probabilityPerApp+=str(getProbabilities(fig, ax1, tempTPSall, chooseName(nodeType), '-o', chooseColor(nodeType),nodeType,missingDataVIP,missingDatacVIP))
        probabilityPerApp+=","
        print(probabilityPerApp)

        #threshold.append(partialCDFPlotData(fig, ax1, tempTPSall, chooseName(nodeType), '-o', chooseColor(nodeType)))
    #print(appThreshold)    
    probabilityPerApp+="\n"
    f = open('meanTPperApp.csv', 'a')
    f.write(probabilityPerApp)
    #runfile.write("./runAndExportSimConfigWithCleanupSH.sh -i omnetpp.ini -c %s -s 5\n" % cfgName )
    f.close()
    ax1.set_xlim(0,1.01*maxTPS)
    #print("in")
    partialCDFEnd(fig,ax1,'', 'cdf Client Throughput [mbps]', '../exports/plots/'+makeFullScenarioName(testName, numCLI, nodeTypes, nodeSplit)+'/'+str(globalCounter)+'_cdfMean'+direction[0]+'ThroughputsCutoff'+ str(cutoff) + str(nodeTypes) + '.pdf')
    partialCDFEndPNG(fig,ax1,'', 'cdf Client Throughput [mbps]', '../exports/plots/'+makeFullScenarioName(testName, numCLI, nodeTypes, nodeSplit)+'/'+str(globalCounter)+'_cdfMean'+direction[0]+'ThroughputsCutoff'+ str(cutoff) + str(nodeTypes) + '.png')

def countMissingTP(testNameMain,numCLI,nodeTypes,nodeSplit,nodeType,nodeNum,direction):
    df = importDFextended(testNameMain, numCLI, nodeTypes, nodeSplit, 'throughputs', '_' + direction[0])
    ...
    df['MISSING'] = np.nan
            #print("How many entries are there in the column for each client")
    colName = direction[0] + " Throughput " + makeNodeIdentifier(nodeType, nodeNum)
 

    if df[colName].isna().sum():
        return 1
    else:
        return 0
            #print(df['MISSING'])
...
def meanProb(testName, numCLI, nodeTypes, nodeSplit, direction, cutoff):
    df = importDFextended(testName, numCLI, nodeTypes, nodeSplit, 'throughputs', '_' + direction[0])
    fig, ax1 = partialCDFBegin(1)
    maxTPS = 12
    threshold=[]
    ...
    for nodeType,numNodes in zip(nodeTypes,nodeSplit):
        ...
        for nodeNum in range(numNodes):
            ...
            #tempTPSall.extend([x/1000 for x in df[colName].tolist()[:int(cutoff)+1]])
            if not countMissingTP(testName,numCLI,nodeTypes,nodeSplit,nodeType,nodeNum,direction):
                tempTPSall.append(statistics.mean([x/1000 for x in df[colName].tolist()[:int(cutoff)+1]])) #tempTPSall will keep mean values for all clients 
            else:
                tempTPSall.append(0)
...
def meanDelayProbability(testName, numCLI, nodeTypes, nodeSplit, dataIdent, folderName, nodeTypesToPlot):
    df = importDF(testName, numCLI, nodeTypes, nodeSplit, folderName)
    fig, ax1 = partialCDFBegin(1)
    maxValue = 0
    ...
    for nodeType,numNodes in zip(nodeTypes,nodeSplit):
        if nodeType in nodeTypesToPlot:
            ...
            print(nodeType)
            print(tempValue)    
            probabilityPerApp+=str(getProbabilitiesDelay(fig, ax1, tempValue, chooseName(nodeType), '-o', chooseColor(nodeType),nodeType,numNodes,counter))
            probabilityPerApp+=","
            print(counter)   
    probabilityPerApp+="\n"
    f = open('meanDelay.csv', 'a')
    f.write(probabilityPerApp)
    #runfile.write("./runAndExportSimConfigWithCleanupSH.sh -i omnetpp.ini -c %s -s 5\n" % cfgName )
    f.close()     
    print(probabilityPerApp)
...
def getProbabilitiesDelay(fig, ax, data, label, lineType, lineColor, hostType, hostNum, counter):
    threshold20=OrderedDict([('hostSSH', 0.01), ('hostVIP',    0.03*1.05), ('hostcVP',   0.03*1.05)])
    ...
    for i in range(0, len(data)):
        if float(data[i]) > threshold20[hostType]:
            counterUnderThreshold+=1
            #print(counterUnderThreshold)

    print(float((counterUnderThreshold+counter)/hostNum*100))    
    return float((counterUnderThreshold+counter)/hostNum*100)

def meanDelayProbabilityV2(testName, numCLI, nodeTypes, nodeSplit, dataIdent, folderName, nodeTypesToPlot):
    df = importDF(testName, numCLI, nodeTypes, nodeSplit, folderName)
    ...
    for nodeType,numNodes in zip(nodeTypes,nodeSplit):
        if nodeType in nodeTypesToPlot:
            ...
            # print(tempValue)
                        allProbabilities.append(getProbabilitiesDelayV2(fig, ax1, tempValue, chooseName(nodeType), '-o', chooseColor(nodeType),nodeType,numNodes,counter))
                    else: 
                        counter+=1
                        
                        allProbabilities.append(100)        
            ...
            print(nodeType)
            #
            print(statistics.mean(allProbabilities))
            probabilityPerApp+=str(statistics.mean(allProbabilities))
            probabilityPerApp+=","
            #print(tempValue)    
            print("Counter")    
            print(counter)       
...
def getProbabilitiesDelayV2(fig, ax, data, label, lineType, lineColor, hostType, hostNum, counter):
    #threshold=OrderedDict([('hostSSH', 0.0101), ('hostVIP', 0.0300428913119999 ), ('hostcVP',   0.0300428913119999)])#0.030027763449999974
    threshold=OrderedDict( [('hostSSH', 0.01), ('hostVIP',    0.03*1.05), ('hostcVP',   0.03*1.05)])#0.030027763449999974
    ...
    counterUnderThreshold = 0
    for i in range(0, len(data)):
        if float(data[i]) > threshold[hostType]:
            
            counterUnderThreshold+=1 
        
    return float((counterUnderThreshold)/len(data)*100)
    mosProbability(testName, numCLI, ["hostVID",'hostLVD', 'hostFDO', 'hostSSH', 'hostVIP','hostcVP','hostcF','hostcLV'], [135,37,60,100,350,100,20,18], 'Mos', 'mos2', ['hostLVD', 'hostFDO', 'hostSSH', 'hostVIP','hostcVP','hostcF','hostcLV'])

def mosProbability(testName, numCLI, nodeTypes, nodeSplit, dataIdent, folderName, nodeTypesToPlot):
    df = importDF(testName, numCLI, nodeTypes, nodeSplit, folderName)
    fig, ax1 = partialCDFBegin(1)
    maxValue = 0
    probabilityPerApp = testName + ","
    nodeSplit.pop(0)
    for nodeType,numNodes in zip(nodeTypesToPlot,nodeSplit):
        if nodeType in nodeTypesToPlot:
            print(nodeType)
            tempValue = []
            allProbabilities=[]
                ...
                if colName in df: 
                   # print(df[colName])
                    ...
                    if len(data) > 0:
                        ...
                        tempValue.extend(data)
            # print(tempValue)
                        allProbabilities.append(getProbabilitiesMosUnderThreshold(fig, ax1, tempValue, chooseName(nodeType), '-o', chooseColor(nodeType),nodeType,numNodes,counter))
                    else: 
                        counter+=1
                        print("COUNTER!")
                        
                        allProbabilities.append(0)        
...
def getProbabilitiesMosUnderThreshold(fig, ax, data, label, lineType, lineColor, hostType, hostNum, counter):
    #threshold=OrderedDict([('hostSSH', 0.0101), ('hostVIP', 0.0300428913119999 ), ('hostcVP',   0.0300428913119999)])#0.030027763449999974
    threshold=OrderedDict([('hostSSH', 3.5), ('hostVIP', 3.5), ('hostcVP',   3.5), ('hostcLV',   3.5),('hostLVD',   3.5), ('hostFDO',   3.5), ('hostcF',   3.5)])
    #print(data)
...
if __name__ == "__main__":
    ...
    numVIP = int(name.split('VIP')[1].split('_cVP')[0])
    numcVP = int(name.split('cVP')[1].split('_cF')[0])
    numcF = int(name.split('cF')[1].split('_cLV')[0])
    numcLV= int(name.split('cLV')[1])
    print(numVID)
    total=numVIP+numSSH+numFDO+numLVD+numVID+numcVP+numcF+numcLV
    #calculateMOS(sys.argv[1], total, ['hostVID', 'hostLVD', 'hostFDO', 'hostSSH', 'hostVIP','hostcVP','hostcF','hostcLV'], [numVID, numLVD, numFDO, numSSH, numVIP,numcVP, numcF, numcLV], 'Mos', 'mos2', ['hostVID', 'hostLVD', 'hostFDO', 'hostSSH', 'hostVIP','hostcVP','hostcF','hostcLV'])
    plotAll(sys.argv[1], '', ['hostVID', 'hostLVD', 'hostFDO', 'hostSSH', 'hostVIP','hostcVP','hostcF','hostcLV'], [numVID, numLVD, numFDO, numSSH, numVIP,numcVP, numcF, numcLV], int(sys.argv[2]), 90)
    #getMeanDataTypeAppClass(sys.argv[1], total, ['hostVID', 'hostLVD', 'hostFDO', 'hostSSH', 'hostVIP'],[numVID, numLVD, numFDO, numSSH, numVIP], 'Mos', 'mos', ['hostVID', 'hostLVD', 'hostFDO', 'hostSSH', 'hostVIP'], {'sliceSSH' : {'hostSSH' : numSSH}, 'sliceVIP' : {'hostVIP' : numVIP}, 'sliceVID' : {'hostVID' : numVID}, 'sliceLVD' : {'hostLVD' : numLVD}, 'sliceFDO' : {'hostFDO' : numFDO}})
