...
def importDF(testName, numCLI, nodeTypes, nodeSplit, nodeType, nodeNum):
    ...
    # Read the CSV
    if not os.stat(fileToRead).st_size == 0: 
        return pandas.read_csv(fileToRead)
    else:
        print("---------------import DF returns empty file-------")
        print(makeNodeIdentifier(nodeType, nodeNum))
        return [makeNodeIdentifier(nodeType, nodeNum)]
...
def zeroThrougputPerSecondDirection(direction, nodeIdent):
    colName = direction[0] + ' Throughput ' + nodeIdent
    tpDirDF = pandas.DataFrame(columns=[colName])
    tpDirDF = tpDirDF.append({colName : 0}, ignore_index=True)
    return tpDirDF
...
def extractNodeThroughputDirection(testName, numCLI, nodeTypes, nodeSplit, nodeType, nodeNum, direction):
    ...
    if type(nodeDF) != list: 
        ...
        print("***********NODE EMPTY************")
        return zeroThrougputPerSecondDirection(direction, nodeIdent)
...
def extractAll(testName, nodeTypes, nodeSplit, numSlices):
    ...
    # if 'hostcVP' in nodeTypes:
    #     extractAllDataTypeToDF(testName, numCLI, nodeTypes, nodeSplit, ['hostcVP'], 'endToEndDelay', 'E2ED', 'e2ed')
    #     extractAllDataTypeToDF(testName, numCLI, nodeTypes, nodeSplit, ['hostcVP'], 'packetLossRate', 'PkLR', 'pklr')
    #     extractAllDataTypeToDF(testName, numCLI, nodeTypes, nodeSplit, ['hostcVP'], 'playoutDelay', 'PlDel', 'pldel')
    #     extractAllDataTypeToDF(testName, numCLI, nodeTypes, nodeSplit, ['hostcVP'], 'playoutLossRate', 'PlLR', 'pllr')
    #     extractAllDataTypeToDF(testName, numCLI, nodeTypes, nodeSplit, ['hostcVP'], 'taildropLossRate', 'TDLR', 'tdlr')
    ...
    #     extractAllDataTypeToDF(testName, numCLI, nodeTypes, nodeSplit, ['hostcLV'], 'DASHliveDelay', 'DLVD', 'dlvd')
...
if __name__ == "__main__":
    ...
    extractAll(sys.argv[1], ['hostVID', 'hostLVD', 'hostFDO', 'hostSSH', 'hostVIP', 'hostcVP','hostcF','hostcLV'], [numVID, numLVD, numFDO, numSSH, numVIP,numcVP, numcF, numcLV], int(sys.argv[2]))